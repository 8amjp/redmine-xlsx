  ...
  function importExcel(e) {
    getWorkbook(e)
      .then(function (workbook) {
        ...
        let enumerations = JSON.parse(core.dataset.enumerations);
        let sheet =workbook.sheet(0);
        // 基本フィールドのデータ
        Object.keys(map).forEach(function(key) {
          switch (key) {
            // project/tracker/author/assigned_to: 何もしない
            case 'project':
            case 'tracker':
            case 'author':
            case 'assigned_to':
            ...
            case 'status':
              if (enumerations.issue_statuses) {
                var values = enumerations.issue_statuses.filter( function(e) {
                  return (e.name == sheet.cell(map[key].cell).value());
                });
                if (values.length > 0) postdata.issue[`${key}_id`] = values[0].id;
              }
              break;
            case 'priority':
              if (enumerations.issue_priorities) {
                var values = enumerations.issue_priorities.filter( function(e) {
                  return (e.name == sheet.cell(map[key].cell).value());
                });
                if (values.length > 0) postdata.issue[`${key}_id`] = values[0].id;
              }
              break;
            case 'category':
              if (enumerations.issue_categories) {
                var values = enumerations.issue_categories.filter( function(e) {
                  return (e.name == sheet.cell(map[key].cell).value());
                });
                if (values.length > 0) postdata.issue[`${key}_id`] = values[0].id;
              }
              break;
            case 'fixed_version':
              if (enumerations.versions) {
                var values = enumerations.versions.filter( function(e) {
                  return (e.name == sheet.cell(map[key].cell).value());
                });
                if (values.length > 0) postdata.issue[`${key}_id`] = values[0].id;
              }
            ...
            case 'total_spent_hours':
              ...
              if (value) postdata.issue[key] = value;
              break;
            // テキスト型
            case 'subject':
            case 'description':
